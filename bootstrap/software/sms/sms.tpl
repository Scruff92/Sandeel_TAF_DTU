...
DATA_SECTION
 ...
 !! cout<<"SMS version January 2018 using ADMB version 12"<<endl;
...
PARAMETER_SECTION
 ...
 matrix    Mean_Fsum(first_VPA,nsp,fyModel,lpy)        // Mean  F from the sum of seasonal F
...
//lower and upper value for stomach variance parameter 
 ...
 sdreport_matrix avg_log_F(first_VPA,nsp,lyModel-sdReportYear,lyModel)
  !! for (s=first_VPA;s<=nsp;s++) for (y=lyModel-sdReportYear;y<=lyModel;y++) {parNo++;  parexp<<"avg_log_F "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1"<<endl; }
  //matrix avg_log_F(first_VPA,nsp,lyModel-sdReportYear,lyModel)
  
  
  sdreport_matrix avg_sumF(first_VPA,nsp,lyModel-sdReportYear,lyModel)
  !! for (s=first_VPA;s<=nsp;s++) for (y=lyModel-sdReportYear;y<=lyModel;y++) {parNo++;  parexp<<"avg_sumF "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1"<<endl; }
  //matrix avg_sumF(first_VPA,nsp,lyModel-sdReportYear,lyModel)

 sdreport_matrix avg_log_sumF(first_VPA,nsp,lyModel-sdReportYear,lyModel)
  !! for (s=first_VPA;s<=nsp;s++) for (y=lyModel-sdReportYear;y<=lyModel;y++) {parNo++;  parexp<<"avg_log_sumF "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1"<<endl; }
  //matrix avg_log_sumF(first_VPA,nsp,lyModel-sdReportYear,lyModel)
 ...
 sdreport_matrix hist_log_SSB(first_VPA,nsp,lyModel-sdReportYear,lyModel) 
 !! for (s=first_VPA;s<=nsp;s++) for (y=lyModel-sdReportYear;y<=lyModel;y++) {parNo++;  parexp<<"hist_log_SSB "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1"<<endl; }
  //matrix hist_log_SSB(first_VPA,nsp,lyModel-sdReportYear,lyModel)
 ...
 //matrix next_log_SSB(first_VPA,nsp,lyModel+1,lyModel+1);
 sdreport_matrix next_log_SSB(first_VPA,nsp,lyModel+1,lyModel+1);
 !! for (s=first_VPA;s<=nsp;s++)  for (y=lyModel+1;y<=lyModel+1;y++) {parNo++;  parexp<<"next_log_SSB "<<parNo<<" "<<s<<" "<<y<<" "<<fq<<" -1 -1 -1 -1 -1"<<endl; }
 ...
 sdreport_matrix rec_sd(first_VPA,nsp,lyModel-sdReportYear,lyModel);
  ...
  sdreport_matrix log_recsd(first_VPA,nsp,lyModel-sdReportYear,lyModel);
  !!for (s=first_VPA;s<=nsp;s++) for (y=lyModel-sdReportYear;y<=lyModel;y++) {parNo++;  parexp<<"log_recsd "<<parNo<<" "<<s<<" "<<y<<" "<<recq<<" -1 "<<fa<<" -1 -1 -1"<<endl; }
  //matrix rec_sd(first_VPA,nsp,lyModel-sdReportYear,lyModel);

   
  // Geometric recruitment using the  ful time series of estimated recruits minus one
...
PROCEDURE_SECTION 
 ...
 calc_avg_F();     //mean F (annual) and mean F from sum of seasonal F)
...
FUNCTION move_recruitment_to_sdreport
 int s,y,yq,q;
  ...
  for (y=lyModel-sdReportYear;y<=lyModel;y++) {
    CALC_yq
    for (s=first_VPA;s<=nsp;s++) {
       rec_sd(s,y)=N(yq,s,fa);
       log_recsd(s,y)=log(N(yq,s,fa));
    }
  }
 ...
 for (s=first_VPA;s<=nsp;s++){
   for (y=fyModel;y<=lyModel;y++){
     ...
     if (sd_phase() && (y>=lyModel-sdReportYear)) {       
       ...
       if (Mean_F(s,y)>0) avg_log_F(s,y)=log(Mean_F(s,y));  else avg_log_F(s,y)=-9;
     }
   }
 }
  for (s=first_VPA;s<=nsp;s++){        // mean F calculated as sum F
   for (y=fyModel;y<=lyModel;y++){
     tmp=0.0;
     for (a=avg_F_ages(s,1);a<=avg_F_ages(s,2);a++) {
        if (a==fa)rc=recq;
     ...
     Mean_Fsum(s,y)=tmp/(avg_F_ages(s,2)-avg_F_ages(s,1)+1);
    if (sd_phase() && (y>=lyModel-sdReportYear)) { 
        avg_sumF(s,y)=Mean_Fsum(s,y);
        if (avg_sumF(s,y)>0) avg_log_sumF(s,y)= log(Mean_Fsum(s,y));    else avg_log_sumF(s,y)=-9;
    }
   }
 }
...
FUNCTION calc_term_N
  ...
  for (s=first_VPA;s<=nsp;s++) {
    ...
    for (a=faq(q);a<=la_VPA(s);a++) {
      if (lqly==lq) {     
      //  cout<<"sp:"<<s<< "  age:"<<a<<" N"<<setprecision(0)<<setw(15)<<N((lyModel+1-fyModel)*lq+fq,s,a)<<setprecision(5)<<
      //       " west:"<<west((lyModel+1-fyModel)*lq+fq,s,a)<<" promat:"<<propmat((lyModel+1-fyModel)*lq+fq,s,a)<<endl;
        SSB(s,lyModel+1)+=N((lyModel+1-fyModel)*lq+fq,s,a)*west((lyModel+1-fyModel)*lq+fq,s,a)*propmat((lyModel+1-fyModel)*lq+fq,s,a);
        TSB(s,lyModel+1)+=N((lyModel+1-fyModel)*lq+fq,s,a)*west((lyModel+1-fyModel)*lq+fq,s,a);
      } else {
  ...
  for (s=first_VPA;s<=nsp;s++){
    ...
    next_log_SSB(s,lyModel+1)=log(SSB(s,lyModel+1));
...
FUNCTION calc_hist_SSB
  ...
  for (y=lyModel-sdReportYear;y<=lyModel;y++){
    for (s=first_VPA;s<=nsp;s++){  
      hist_SSB(s,y)=SSB(s,y);
      hist_log_SSB(s,y)= log(hist_SSB(s,y));
    }
  }
 // q=fq;
 // for (y=lyModel-sdReportYear;y<=lyModel;y++){
...
FUNCTION void print_summary_table()
 ...
 for (s=first_VPA;s<=nsp;s++){
    ...
    for (y=fyModel;y<=lyModel;y++){
      ...
      res<< setw(15) << setprecision(3) << setfixed() <<Mean_Fsum(s,y)<<endl;
    }
 

    res<< setw(36) << setprecision(0) << setfixed() <<SSB(s,lyModel+1)<<endl;
 }
 res.close();
 if (multi==2) calc_eaten_M2_hist();
 ofstream res2("summary_table_raw.out",ios::out);
 res2<<"Species.n Year  Rec SSB TSB SOP SOP.hat Yield Yield.hat mean.F Eaten Mean.Fsum Mean.Fanu"<<endl;
 ...
 for (s=first_VPA;s<=nsp;s++){
    for (y=fyModel;y<=lyModel;y++){
      ...
      res2<<s<<" "<<y<<"  "<<N(yq,s,fa)<<" "<<SSB(s,y)<<" "<<
            TSB(s,y)<<" "<<CWsum(s,y)<<" "<<CWsum_hat(s,y)<<" "<<yield(s,y)<<" "<<yield_hat(s,y)<<" "<<Mean_Fsum(s,y)<<" "<<eaten_M2(s,y)<<" "<<Mean_Fsum(s,y)<<" "<< Mean_F(s,y)<<endl;
    }
  ...
  //  }
       res2<<s<<" "<<lyModel+1<<"  NA "<<SSB(s,lyModel+1)<<" NA NA NA NA NA NA NA NA NA"<< endl;
