...
plot.survey.residuals2<-function(dev,nox=1,noy=1,Portrait=T,start.year=0,end.year=0,reverse.colors=F,use.ref.dot=TRUE,add.title=TRUE,over.all.max=1.5,my.species=NA,standardize=F,...) {
...
for (sp in 1:nsp) if (sp %in% my.species) {
  ...
  if (!markD) print(paste(sp.name,sp,"Number of fleets:",nf))

  for (f in 1:nf) {

    if (!markD) print(paste("sp:",sp,"  fleet:",f))
    ...
    if (plot.no%%nox.noy==0 || f==1){
     newplot(dev,nox,noy,Portrait=Portrait,filename=paste('Survey',sp,f,plot.no),...)
      par(mar=c(3,4,3,2))
