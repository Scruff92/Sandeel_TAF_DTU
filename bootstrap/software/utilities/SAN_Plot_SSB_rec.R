...
recruit.years<-matrix(scan(file=file.path(data.path,'recruitment_years.in'),comment.char='#',quiet=T),nrow=dim(rec)[1],byrow=T)
...
for (sp in ss) {
  ...
  if (i==noxy) {newplot(dev,filename=paste('SSB_R_a.fit',s.index,sep=''),nox,noy,Portrait); i<-0 }
...
if (My.device =='screen') savePlot(filename= file.path(mdDir,"SSB_rec.png"),type="png")
